...
def printrbot_v2_gcode_converter(script):
...
try:
 ...
 opts, extraparams = getopt.getopt(sys.argv[1:],'eto:fo:fmini:fmino:fmaxo:mint:f',['eto=','fanOutput=','fanMinInValue=','fanMinOutValue=', 'fanMaxOutValue=', 'minTraverseSpeed=', 'file='])
 ...
 for o,p in opts:
  if o in ['-eto','--eto']:
   eto = p
  elif o in ['-fo','--fanOutput']:
   fanOutput = int(p)
  elif o in ['-fmini','--fanMinInValue']:
   fanMinInValue = float(p)
  elif o in ['-fmino','--fanMinOutValue']:
   fanMinOutValue = float(p)
  elif o in ['-fmaxo','--fanMaxOutValue']:
   fanMaxOutValue = float(p)
  elif o in ['-mint','--minTraverseSpeed']:
   minTraverseSpeed = float(p)
  elif o in ['-f','--file']:
   filename = p
 if not filename:
  printrbot_v2_gcode_converter(inspect.stack()[0][1])
...
with open(filename, "r") as f:
    ...
    with open(filename, "w") as f:
        ...
        for line in lines:
            ...
            # Fix M82
            #Added in Simplify3D, not needed
            line_split[0] = re.sub(r"^[Mm]82\b(.*)$", ";M82\\1", line_split[0])
